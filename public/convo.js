...
async function initApp(force=false){if(!currentUser||appLoading&&!force)return;appLoading=true;$('userDisplayName').textContent=currentUser.name||'';$('btnCreateChannel').style.display='inline-flex';$('superAdminLauncher').hidden=currentUser.role!=='superadmin';closeSuperAdminPanel();showAppBoot('LOADING CONVO','Preparing your workspace…');const cached=readCachedChannels();if(cached.length){currentChannels=cached;renderChannels()}if(!currentChannels.length){currentChannels=[{id:'general-local',name:'general',description:'Common community thread'}];renderChannels()}const initial=currentChannels.find(c=>c.name==='general')||currentChannels[0];if(initial){activeChannel=initial;$('activeChannelHeading').textContent=`# ${initial.name}`;$('activeChannelDesc').textContent=initial.description||'Project discussion';renderChannels();$('messagesFeed').innerHTML='<div class="state-message">Loading messages...</div>'}setTimeout(()=>hideAppBoot(),320);loadChannels(null,false).then(async target=>{if(!target)return;await selectChannel(target.name,false)}).catch(err=>{console.warn('Workspace sync failed after shell load.',err)}).finally(()=>{if(currentUser?.role==='superadmin')Promise.allSettled([loadAdminUsers(),loadAdminAnalytics()])});appLoading=false}
