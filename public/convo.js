...
window.deleteCurrentGroup=async function(){if(!activeChannel||activeChannel.name==='general'||!['admin','superadmin'].includes(currentUser?.role))return;if(!confirm(`Delete #${activeChannel.name} and its messages?`))return;try{await api('/channels/delete',{method:'POST',body:JSON.stringify({id:activeChannel.id})});activeChannel=null;await loadChannels('general')}catch(err){alert(err.message||'Unable to delete channel.')}};
...
async function initApp(){if(!currentUser||appLoading)return;appLoading=true;$('userDisplayName').textContent=currentUser.name||'';$('btnCreateChannel').style.display='inline-flex';$('superAdminLauncher').hidden=currentUser.role!=='superadmin';closeSuperAdminPanel();try{await loadChannels();if(currentUser.role==='superadmin'){await loadAdminUsers();await loadAdminAnalytics()}}catch(err){$('messagesFeed').innerHTML='<div class="state-message">Unable to connect to Convo.</div>';showAppError(err)}finally{appLoading=false}}
