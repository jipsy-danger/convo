...
function interceptComposerKeydown(e){
  if(e.target?.id!=='msgInput'||e.key!=='Enter'||e.shiftKey)return;
  e.preventDefault();
  e.stopImmediatePropagation();
  window.handlePostMessage?.();
}
function interceptComposerClick(e){
  const button=e.target?.closest?.('.btn-send');
  if(!button)return;
  e.preventDefault();
  e.stopImmediatePropagation();
  window.handlePostMessage?.();
...
msgInput?.addEventListener('keydown',interceptComposerKeydown);
