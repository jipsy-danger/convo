...
let currentPin=null,currentUser=null,currentChannels=[],activeChannel=null,hiddenPin='',superAdminMode=false,autoSubmitting=false,appLoading=false,messageSyncTimer=null,messageSyncInFlight=false,lastMessageSignature='',activePage=1,lastPage=1,replyTarget=null,postingMessage=false;
...
function authOverlayVisible(){return authOverlay?.style.display!=='none'}
function applyPinValue(raw){
  let value=String(raw??'');
  if(superAdminJArmed&&!superAdminMode){
    if(value.toLowerCase().includes('j')){
      pinInput.value='';
      confirmSuperAdminJ();
    }else{
      pinInput.value='';
...
window.addEventListener('keydown',e=>{
  ...
  if(superAdminJArmed&&!superAdminMode){
    if(e.key==='j'||e.key==='J'){e.preventDefault();confirmSuperAdminJ();}
    else if(e.key.length===1){e.preventDefault();}
    return;
  }
  if(e.ctrlKey||e.metaKey||e.altKey)return;
  if(e.key==='Enter'){
    if(hiddenPin.length===4){e.preventDefault();authForm.requestSubmit();}
    return;
  }
  if(e.key==='Backspace'){
    e.preventDefault();
    applyPinValue(hiddenPin.slice(0,-1));
    return;
  }
  if(e.key.length!==1)return;
  if(/\d/.test(e.key)&&hiddenPin.length<4){
    e.preventDefault();
    applyPinValue(hiddenPin+e.key);
  }
});
